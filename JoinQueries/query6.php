...
<body>
    ...
    <div class = container>
        <div class = "row" >
            ...
            <div class="col-lg-6">
            <h2><span class="label label-default pull-right">Enter the Account Id to get the List of Address associated with the Account</span></h2></div>
            <div class="col-lg-3"></div>
        </div>
        <hr/>    
                ...
                <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>"> 
                    <div class="input-group">
                      <span class="input-group-btn">
                        <button class="btn btn-default" type="submit">Display Associated Address</button>
                    </span>
    ...
    <?php include '../orm/accountAddress.php'; ?>
    ...
    try {
        if(!empty($_REQUEST['accountId'])) {
            ...
            $sql_stmt = "SELECT addr.ID, addr.UNITNUMBER, addr.STREETNAME, addr.CITY, addr.STATE, addr.COUNTRY, addr.ZIPCODE FROM address_account addacc, ACCOUNT acc, address addr WHERE addacc.ACCOUNTID = acc.Id AND addr.ID = addacc.ADDRESSID AND acc.Id = :accountId ;";

            $sql = $dbh->prepare($sql_stmt);

            if($sql->execute(array(':accountId' => $accountId))) {
                $sql->setFetchMode(PDO::FETCH_ASSOC);
				$sql->setFetchMode(PDO::FETCH_CLASS, "AccountAddress");
            }	
        }
    }   
            ...
            <div id = "table-wrapper" class= "table-responsive"> 
                <table class="table table-striped">
                    ...
                    <?php while($flag == TRUE and $modeAddressAccount = $sql->fetch()) { ?>
                        <?php
							createAccountAddressRow($modeAddressAccount) 
						?>
                    <?php } ?>
                    </tbody>
                </table>
